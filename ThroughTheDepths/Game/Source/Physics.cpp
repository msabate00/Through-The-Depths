		...
		for (b2Body* b = world->GetBodyList(); b; b = b->GetNext())
		{
			for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
			{
				...
				case b2Shape::e_circle:
				{
					...
					app->render->DrawCircle(METERS_TO_PIXELS(pos.x * app->win->GetScale()) + app->render->camera.x , METERS_TO_PIXELS(pos.y * app->win->GetScale()) +app->render->camera.y , METERS_TO_PIXELS(shape->m_radius) * app->win->GetScale(), 255, 255, 255);
				}
				break;
